...
@DisplayName("Decorator Pattern Scala 2 Tests")
class ReportExporterTest {
  ...
  def shouldExportPlainTextUnchanged(): Unit = {
    val exporter: ReportExporter = new PlainTextReportExporter
  ...
  def shouldCompressExportedContent(): Unit = {
    val exporter: ReportExporter = new CompressionDecorator(new PlainTextReportExporter)
  ...
  def shouldEncryptAndDecryptRoundTrip(): Unit = {
    val exporter: ReportExporter = new EncryptionDecorator(new PlainTextReportExporter, 42)
  ...
  def shouldRecordAuditLogWhenExporting(): Unit = {
    ...
    val exporter: ReportExporter = new AuditLoggingDecorator(new PlainTextReportExporter, auditLog)
  ...
  def shouldPublishSensitiveReportWithFullDecoratorChain(): Unit = {
    val auditLog          = ArrayBuffer.empty[String]
    val publishingService = new ReportPublishingService(auditLog)

    val published = publishingService.publish(
      report,
      compress      = true,
      encrypt       = true,
      audit         = true,
...
class ReportPublishingService(auditLog: ArrayBuffer[String]) {
  def publish(
    ...
    var exporter: ReportExporter = new PlainTextReportExporter
    if (compress) exporter = new CompressionDecorator(exporter)
    if (encrypt) exporter  = new EncryptionDecorator(exporter, encryptionKey)
    if (audit) exporter    = new AuditLoggingDecorator(exporter, auditLog)
